...
/** True if either side is a Holding interface (V1 or V2). */
export function isHoldingInterfaceId(interfaceId: string): boolean {
    return (
        matchInterfaceIds(HOLDING_INTERFACE_ID, interfaceId) ||
        matchInterfaceIds(HOLDING_INTERFACE_ID_V2, interfaceId)
    )
}

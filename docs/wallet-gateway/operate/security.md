...
- **Use OAuth / OpenID Connect IDPs.** Reserve self-signed tokens for development. See
  [Networks & identity providers](networks-and-identity.md#identity-providers).
...
- **If you must run a signing store,** restrict filesystem permissions, consider encryption at
  ...
  [Signing store security](../reference/configuration-reference.md#signing-store-security).

## Network exposure

- **Restrict CORS.** Set `server.allowedOrigins` to known dApp origins instead of `"*"`. See
  [Configure the Wallet Gateway](configure.md#server).
...
- **Use PostgreSQL** and take regular, tested backups. See
  [Backups and recovery](../reference/configuration-reference.md#backups-and-recovery).
- **Keep secrets out of config files.** Supply `clientSecret`, `adminAuth`, and provider API
  ...
  [Secrets and environments](../reference/configuration-reference.md#secrets-and-environments).

...
Automation only reuses stored sessions when the access token is a valid **client-credentials** JWT (`gty: client_credentials`), so a short-lived interactive OAuth token from the User UI is not picked up by the Signing worker or M2M `prepareExecute` bootstrap.

...
You are 100% responsible for every contribution you make under your name, regardless of which tools were involved in producing it. Reviewers review _your_ work, not AI output.

You must understand every line of code you submit and be able to explain _why_ each change is there. "The AI wrote it" is not an answer.
...
If you are picking up a `good first issue` or similar onboarding-oriented task, the task exists partly so that _you_ learn the codebase. Using AI to speed up completion of the task may defeat the purpose — for yourself and for the maintainers trying to onboard you. Engage with the code.
